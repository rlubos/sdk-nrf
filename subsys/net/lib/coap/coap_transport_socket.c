...
/**@brief Internal method to find the session or port corresponding to the handle.
 *
 * @param[in] handle Identifies the socket descriptor, also the the handle for the transport.
 *
...
{
    index < COAP_PORT_COUNT ? false : true;
}
			   ...
			   u16_t datalen)
{
    ...
    int32_t index = local_endpoint_find(transport);
    if (index == -1) {
    	err_code = EBADF;
    } else {    	
...
u32_t coap_security_setup(coap_local_t *local, struct sockaddr const *remote)
{
	...
	if (session != NULL) {
		local->transport = session->local.transport;
	...
	for (int index = 0; index < COAP_SESSION_COUNT; index++) {
		...
		if (session->local == NULL) {
			...
			if (transport != -1) {
				session->local.transport = transport;
...
u32_t coap_security_destroy(coap_transport_handle_t transport)
{
	...
	if (secure_endpoint_check(index))
	{
		session_t   *session = &m_session[index - COAP_PORT_COUNT];
