...
static int handle_at_socket(const char *at_cmd_param);
static int handle_at_bind(const char *at_cmd_param);
static int handle_at_tcp_conn(const char *at_cmd_param);
static int handle_at_tcp_send(const char *at_cmd_param);
static int handle_at_tcp_recv(const char *at_cmd_param);
static int handle_at_udp_sendto(const char *at_cmd_param);
static int handle_at_udp_recvfrom(const char *at_cmd_param);
...
static int handle_at_socket(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		if (op == 1) {
			...
			err = at_params_short_get(&m_param_list, 1, &type);
			if (err < 0) {
				return err;
			};
			if (client.sock > 0) {
...
static int handle_at_bind(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		err = at_params_string_get(&m_param_list, 0, ip, &size);
		if (err < 0) {
			return err;
		};
		ip[size] = '\0';
...
static int handle_at_tcp_conn(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		err = at_params_string_get(&m_param_list, 0, url, &size);
		if (err < 0) {
			return err;
		};
		url[size] = '\0';
...
static int handle_at_tcp_send(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		err = at_params_string_get(&m_param_list, 0, data, &size);
		if (err < 0) {
			return err;
		};
		data[size] = '\0';
...
static int handle_at_tcp_recv(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		err = at_params_short_get(&m_param_list, 0, &length);
		if (err < 0) {
			return err;
		};
		err = at_params_short_get(&m_param_list, 1, &time);
		if (err < 0) {
			return err;
		};
...
static int handle_at_udp_sendto(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		size = NET_IPV4_MTU;
		err = at_params_string_get(&m_param_list, 2, data, &size);
		if (err < 0) {
			return err;
		};
		data[size] = '\0';
...
static int handle_at_udp_recvfrom(const char *at_cmd_param)
{
	...
	if (*(at_param) == '=') {
		...
		err = at_params_short_get(&m_param_list, 3, &time);
		if (err < 0) {
			return err;
		};
